...
class IsTruthHandlerProvider extends HandlerProvider {
  Provide(ctx: PluginContext): void {
    ctx()
      ...
      .action(this.IsTruthCommandHandler(ctx));
  }

  private IsTruthCommandHandler: CommandHandlerFunc = (
    ctx: PluginContext
  ): Command.Action => {
    const SystemPrompt = `你是一位严谨的事实核查员。你的任务是针对用户提供的材料（可能是新闻、实事或是来自社交平台截图，以及OCR识别结果），通过搜索交叉验证多个可信信源(权威媒体、政府机构、学术机构等，搜索社交媒体时注意核验用户账号身份)，核查其真实性。如果材料最后附有用户提问，请优先回答。
回答格式（为适配社交平台，请严格遵守）：
...
要求：内容简洁、干练，全程禁用任何Markdown或其他特殊格式。`;
    ...
    const resolveElements = async (
      ...
      await Promise.all(
        elements.map(async (element) => await resolveElement(ctx, element))
      );
    ...
    return async (input: CommandHandlerInput) => {
      ...
      let content: string;
      try {
        content = (await resolveElements(ctx, elements)).join("\n");
      } catch (error) {
