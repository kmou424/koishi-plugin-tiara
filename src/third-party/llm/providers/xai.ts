...
export interface xAIWebSearchConfig {
  webSearch: true;
  mode: "auto" | "on" | "off";
...
export const xAIWebSearchConfig: Schema<xAIWebSearchConfig> = Schema.object({
  ...
  mode: Schema.union([
    ...
    .description("搜索模式")
    .default("auto"),
  returnCitations: Schema.boolean().description("是否返回引用").default(false),
...
export const xAIConfig: Schema<xAIConfig> = Schema.intersect([
  Schema.object({
    ...
    model: Schema.union([
      ...
      .description("xAI 模型")
      .default("grok-3-mini"),
    webSearch: Schema.boolean().default(false).description("是否启用网络搜索"),
  }),
  Schema.union([
    xAIWebSearchConfig,
    Schema.object({}) as Schema<xAIWebSearchConfig>,
  ]),
]);
